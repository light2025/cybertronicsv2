...
export default function MobileLifestyle({ payload }: Props) {
  const slug = payload?.categorySlug as string | undefined;

  const categories = useDataStore((s) => s.categories);
  const products = useDataStore((s) => s.products);
  const pushMobile = useMobileStore((s) => s.push);
  ...
  const openProduct = (productId: string) =>
    pushMobile({ kind: 'app', appId: 'product-detail', payload: { productId } });
  ...
  if (slug) {
    ...
    return (
      <div className="flex-1 flex flex-col min-h-0 bg-white">
        ...
        >
          <button onClick={popMobile} className="text-[#1a4a8a] font-bold">
            ‹ Categories
          </button>
